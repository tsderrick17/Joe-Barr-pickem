...
The same panel documents the live cadence: the first NFL score check is eligible 170 minutes after kickoff, the worker is invoked every 15 minutes, and unfinished games follow the regular retry ladder (10, 10, 10, 15, 15, 30, 60, 120, then 360 minutes). Playoff games use the faster ladder shown in the panel. Recent score polls list eligible games, completed finals found, finals saved, and credits consumed so actual provider performance can be compared with the policy.
