...
export async function ensureBowlDailyRecapSnapshot(reminderId: string, current: unknown): Promise<BowlDailyRecapSnapshot> {
  ...
  const [{ data: entries, error: entryError }, { data: results, error: resultError }, { data: champions, error: championError }, { data: seasonGames, error: seasonGamesError }] = await Promise.all([
    ...
    supabaseAdmin.from("bowl_pool_games").select("status").eq("season_id", seasonId),
  ]);
  if (entryError || resultError || championError || seasonGamesError) throw new Error("Bowl recap standings could not be read.");
  ...
  const eliminatedToday = rows.filter((row) => row.wins + remainingGames < leaderWins).map((row) => row.name);
