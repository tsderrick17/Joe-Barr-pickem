...
export async function respondToCriticalWorkerHealth(probeName: string, worker?: CriticalWorkerName) {
  const checkedAt = new Date();
  try {
    const result = await checkCriticalWorkerHealth(checkedAt, worker);
    ...
    const unhealthySince = result.healthy ? null : priorState?.unhealthy_since ?? checkedAt.toISOString();
