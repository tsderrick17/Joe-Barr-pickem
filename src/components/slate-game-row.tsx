...
export default function SlateGameRow({ game, alternate, hasStarted, selectedTeamId, allowSelection = false, onChoose }: Props) {
  ...
  const teamCell = (team: typeof left, align: "left" | "right") => {
    ...
    const content = <><span className="inline-block">{label}<ResultMark result={isFinal ? team.result : null} />{isFinal && team.score !== null ? <span className="ml-1 font-mono font-black tabular-nums">{team.score}</span> : null}</span>{hasStarted && team.pickers.length ? <span className={`mt-0.5 block text-[10px] font-semibold leading-3 ${selected ? "text-slate-200" : "text-slate-600"}`}>Picked by {team.pickers.join(", ")}</span> : null}</>;
  ...
  return <article className={`grid grid-cols-[3.25rem_minmax(0,1fr)_6.5rem_minmax(0,1fr)] items-center gap-2 border-b border-[#c8c1b5] py-2 pl-1 pr-3 sm:grid-cols-[4.5rem_minmax(0,1fr)_6.5rem_minmax(0,1fr)] sm:gap-3 sm:pl-2 sm:pr-4 ${alternate ? "bg-[#eee4d1]" : "bg-[#fffdf8]"}`}>
    <div className="text-center text-[10px] font-bold leading-3 text-slate-600 sm:text-xs">
      {isFinal ? <p className="font-mono font-bold text-slate-700">{easternShortDate(game.kickoffAt)}</p> : <><p>{easternTime(game.kickoffAt).replace(" EDT", "").replace(" EST", "")}</p><p className={`mt-1 text-[8px] font-black tracking-[0.1em] ${isLive ? "text-red-700" : "text-slate-500"}`}>{isLive ? "LIVE" : "ET"}</p></>}
    </div>
    {teamCell(left, "left")}
