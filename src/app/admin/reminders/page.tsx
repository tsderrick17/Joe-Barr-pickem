...
type Reminder = {
  id: string; category: string; audience: string; title: string; body: string;
  scheduledFor: string; status: string; delivered: number; failed: number;
  emailDelivered: number; emailFailed: number;
};
...
const presets = [
  { category: "weekly", audience: "all_active", title: "A fresh slate is ready", body: "A new Pick'em week is open. Take a look whenever you have a moment and make your selections before kickoff.", label: "Wednesday fresh slate" },
  { category: "final_lines", audience: "all_active", title: "Today’s official lines are set", body: "The final lines are posted for today’s games. The Slate is ready whenever you are.", label: "Game-day final lines · 8:30 AM" },
  { category: "final_lines", audience: "all_active", title: "Sunday’s official lines are set", body: "Sunday’s final lines are posted. Take a quick look at The Slate before the day gets moving.", label: "Sunday final lines · 8:30 AM" },
  { category: "pick_due", audience: "pick_due", title: "A quick Pick'em check-in", body: "There is still time to take care of anything waiting for you. Open the pool when you are ready.", label: "Sunday courtesy check · 11 AM" },
  { category: "pick_due", audience: "pick_due", title: "One last Pick'em check-in", body: "Before tonight’s game, please take a moment to look over anything still waiting for you.", label: "Monday emergency check · 5 PM" },
...
export default function ReminderAdminPage() {
  ...
  const [form, setForm] = useState({ category: "weekly", audience: "all_active", title: "A fresh slate is ready", body: "A new Pick'em week is open. Take a look whenever you have a moment and make your selections before kickoff.", scheduledFor: initialTime() });
  ...
  return <main className="min-h-screen bg-[#f7f3e8] px-4 py-8 text-zinc-900 sm:px-6 sm:py-10"><div className="mx-auto max-w-4xl">
    ...
    <section className="py-8"><h2 className="font-serif text-2xl font-bold">Reminder receipt log</h2><p className="mt-1 text-sm text-zinc-700">A scheduled item can be cancelled until delivery begins. Each channel blocks duplicate sends independently.</p>{loading ? <p className="mt-5">Loading reminders...</p> : reminders.length === 0 ? <p className="mt-5 text-zinc-700">No reminders have been scheduled yet.</p> : <div className="mt-5 space-y-3">{reminders.map((reminder) => <article className="border border-zinc-400 bg-white p-4" key={reminder.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-zinc-600">{reminder.category.replaceAll("_", " ")} · {reminder.audience.replaceAll("_", " ")} · {reminder.status}</p><h3 className="mt-1 font-bold">{reminder.title}</h3><p className="mt-1 text-sm text-zinc-700">{reminder.body}</p><p className="mt-3 text-sm text-zinc-600">Scheduled {new Date(reminder.scheduledFor).toLocaleString()} · Browser: {reminder.delivered} delivered, {reminder.failed} failed · Email: {reminder.emailDelivered} delivered, {reminder.emailFailed} failed</p></div>{reminder.status === "scheduled" ? <button className="border border-red-800 px-3 py-2 text-sm font-bold text-red-800 disabled:opacity-50" disabled={saving} onClick={() => void cancel(reminder.id)} type="button">Cancel</button> : null}</div></article>)}</div>}</section>
  </div></main>;
}
