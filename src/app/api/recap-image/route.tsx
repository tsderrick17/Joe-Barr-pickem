...
export async function GET(request: NextRequest) {
  ...
  if (kind === "reveal" && snapshot.kind === "sunday_reveal") return new ImageResponse(
    <div style={{ background: "#fffdf8", color: "#171719", display: "flex", flexDirection: "column", height: "100%", padding: "46px 54px", width: "100%" }}>
      <div style={{ borderBottom: "6px solid #171719", display: "flex", justifyContent: "space-between", paddingBottom: 20 }}><span style={{ fontFamily: "Georgia", fontSize: 44, fontWeight: 800 }}>Lead Pipe Locks</span><span style={{ color: "#007e72", fontSize: 20, fontWeight: 800, letterSpacing: 2, paddingTop: 18 }}>SUNDAY {snapshot.window.toUpperCase()}</span></div>
  ...
  if (kind === "reveal" && snapshot.kind === "featured_window_reveal") return new ImageResponse(
    <div style={{ background: "#fffdf8", color: "#171719", display: "flex", flexDirection: "column", height: "100%", padding: "46px 54px", width: "100%" }}>
      <div style={{ borderBottom: "6px solid #171719", display: "flex", justifyContent: "space-between", paddingBottom: 20 }}><span style={{ fontFamily: "Georgia", fontSize: 44, fontWeight: 800 }}>Lead Pipe Locks</span><span style={{ color: "#007e72", fontSize: 20, fontWeight: 800, letterSpacing: 2, paddingTop: 18 }}>FEATURED WINDOW</span></div>
      <div style={{ color: "#475569", display: "flex", fontSize: 19, fontWeight: 800, letterSpacing: 2, marginTop: 18 }}>{snapshot.week.toUpperCase()} · {snapshot.window.toUpperCase()}</div>
      <div style={{ display: "flex", flexDirection: "column", marginTop: 14 }}>{snapshot.rows.map((row, index) => <div key={row.name} style={{ alignItems: "center", background: index % 2 ? "#eee4d1" : "#fffdf8", borderBottom: "1px solid #c8c1b5", display: "flex", fontSize: 22, minHeight: 56, padding: "0 12px" }}><span style={{ display: "flex", fontFamily: "Georgia", fontWeight: 700, width: 170 }}>{row.name}</span><span style={{ display: "flex", flex: 1 }}>{row.picks.join(" · ") || "No public picks yet"}</span><span style={{ color: "#08785d", display: "flex", fontWeight: 800 }}>{row.wins} W</span></div>)}</div>
      <div style={{ borderTop: "3px solid #171719", display: "flex", fontSize: 18, marginTop: "auto", paddingTop: 16 }}>Only selections from games already underway are shown. Future picks remain private.</div>
    </div>,
    { width: 1200, height: 1200 },
