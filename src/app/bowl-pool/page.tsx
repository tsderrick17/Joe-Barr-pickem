...
export default function BowlPoolPage() {
  ...
  return (
    <main className="bowl-pool-page mx-auto max-w-6xl px-2 py-8 sm:px-6 sm:py-10">
      ...
      {!isLoading && canView ? (
        <>
          {poolLocked ? optedIn === false ? <div className="mt-6 border border-slate-300 bg-white p-5 text-center text-sm font-bold text-slate-700">Bowl Pool entry is closed for this year. Check back next year.</div> : null : optedIn === null ? <div aria-busy="true" className="mt-4 flex items-center justify-center gap-3 border border-slate-300 bg-white p-3 text-center text-sm font-bold text-slate-500">Loading…</div> : <label className="bowl-opt-in-bar mt-4 flex items-center justify-center gap-3 border border-slate-300 bg-white px-3 py-2.5 text-center sm:px-4 sm:py-3"><input className="h-4 w-4 shrink-0" type="checkbox" checked={optedIn} onChange={(event) => void changeOptIn(event.target.checked)} /><span className="text-xs font-bold text-slate-700 sm:text-sm">I would like to participate in the NCAA Bowl Pool (you can opt out prior to first kickoff)</span></label>}
