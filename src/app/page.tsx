...
export default function HomePage() {
  ...
  const bowlOwnCell = (game: BowlMatrixGame) => {
    ...
    if (saved) {
      ...
      return team ? bowlTeamLabel(team) : "·";
    }
          ...
          {!bowlPoolMinimized ? <>
            ...
            <div className="bowl-standings-scroll overflow-x-auto border-b-2 border-[#1d1d1f]" ref={bowlScrollRef}>
              <div style={{ minWidth: bowlTableMinWidth }}>
                ...
                <div className="grid" style={{ gridTemplateColumns: `3rem 5rem repeat(${bowlGames.length}, minmax(7.5rem, 1fr)) minmax(6.5rem, .72fr)` }}><span className="bowl-standings-summary-cell bowl-standings-sticky sticky left-0 z-30 flex min-h-28 flex-col items-center justify-center bg-[#f5f0e6] px-1 text-center uppercase text-slate-700" style={{ gridColumn: "span 2" }}><span className="text-[9px] font-black tracking-wide">Games remaining</span><BowlFlipNumber value={bowlScheduleReady ? Math.max(0, bowlGames.length - bowlGradedGames) : null} /></span>{bowlGames.map((game, index) => <span className={`bowl-standings-game-cell flex flex-col items-center bg-[#f7f3ea] px-2 py-1.5 text-center text-[10px] leading-4 text-slate-700 ${bowlGameBoundaryClass(index)}`} data-bowl-game-index={index} key={`${game.id}-${index}`}><b className="bowl-standings-bowl-name flex min-h-8 w-full items-center justify-center text-xs uppercase leading-4 tracking-wide text-slate-900">{bowlName(game)}</b><span className="bowl-standings-team block w-full truncate font-bold text-slate-950" title={bowlTeam(game, "favorite")?.full_name}>{bowlTeamLabel(bowlTeam(game, "favorite"))}</span><span className={`bowl-standings-line block font-mono font-black ${game.line?.locked_at ? "text-[#007e72]" : "text-slate-950"}`}>{game.line?.locked_spread ?? "—"}</span><span className="bowl-standings-team block w-full truncate font-bold text-slate-950" title={bowlTeam(game, "underdog")?.full_name}>{bowlTeamLabel(bowlTeam(game, "underdog"))}</span></span>)}<span className="bowl-standings-tiebreaker-header flex items-center justify-center border-l-2 border-[#8d877d] bg-[#f5f0e6] px-2 py-1.5 text-center text-[10px] font-black uppercase tracking-wide text-slate-700">Tiebreaker</span></div>
                {bowlRows.map((row, rowIndex) => { const rowFill = rowIndex % 2 ? "is-alt bg-[#e9eef4]" : "bg-[#f5f0e6]"; const isViewer = row.playerId === data?.viewerPlayerId; return <div className={`bowl-standings-player-row grid border-b border-[#91afd0] text-center text-xs ${rowIndex === 0 ? "border-t-2 border-t-[#1d1d1f]" : ""} ${rowFill}`} style={{ gridTemplateColumns: `3rem 5rem repeat(${bowlGames.length}, minmax(7.5rem, 1fr)) minmax(6.5rem, .72fr)` }} key={row.playerId}><span className={`bowl-standings-sticky sticky left-0 z-30 px-1 py-2 text-center font-mono text-[13px] font-extrabold tabular-nums sm:text-base ${rowFill}`}>{row.wins}</span><span className={`bowl-standings-sticky sticky left-[3rem] z-30 truncate px-1 py-2 text-left font-serif text-[13px] font-extrabold leading-tight tracking-[0.01em] sm:text-base ${rowFill}`} title={row.playerName}><PlayerTrophyName name={row.playerName} showTrophy={row.trophies?.some((title) => title.includes("Bowl Pool Champion"))} titles={row.trophies} /></span>{bowlGames.map((game, index) => { const result = isViewer ? bowlOwnCell(game) : bowlCell(row.playerId, game.id); const locked = !isViewer && bowlStandings?.privatePickMarkers?.some((pick) => pick.playerId === row.playerId && pick.game_id === game.id); const display = result === "·" && locked ? "🔒" : result; return <span className={`px-1 py-2 font-black ${bowlCellClass(display)} ${bowlGameBoundaryClass(index)}`} key={`${row.playerId}-${game.id}-${index}`}>{display}</span>; })}<span className="flex items-center justify-center border-l-2 border-[#8d877d] px-2 py-2 font-mono text-[13px] font-extrabold tabular-nums text-slate-700 sm:text-base">{row.tiebreakerTotal ?? "—"}</span></div>; })}
              </div>
            </div>
          </> : null}
        </section> : null}
