...
export default function HomePage() {
  ...
  const bowlTableMinWidth = `${13.5 + bowlGames.length * 8}rem`;
          ...
          {!bowlPoolMinimized ? <>
            ...
            <div className="bowl-standings-scroll overflow-x-auto border-b-2 border-[#1d1d1f]" ref={bowlScrollRef}>
              <div style={{ minWidth: bowlTableMinWidth }}>
                <div className="grid" style={{ gridTemplateColumns: `3rem 6rem repeat(${bowlGames.length}, minmax(8rem, 1fr)) minmax(4.5rem, .55fr)` }}><span aria-hidden="true" className="bowl-standings-sticky sticky left-0 z-30 bg-[#e9eef4]" style={{ gridColumn: "span 2" }} />{bowlDateGroups.map((group) => <span className="border-x-2 border-t-2 border-[#8d877d] bg-[#e9eef4] px-2 py-2 text-center text-[10px] font-black uppercase tracking-wide text-slate-600" key={group.key} style={{ gridColumn: `span ${group.count} / span ${group.count}` }}>{group.key}</span>)}<span aria-hidden="true" className="border-t-2 border-[#8d877d] bg-transparent" /></div>
                <div className="grid" style={{ gridTemplateColumns: `3rem 6rem repeat(${bowlGames.length}, minmax(8rem, 1fr)) minmax(4.5rem, .55fr)` }}><span className="bowl-standings-sticky sticky left-0 z-30 flex min-h-28 flex-col items-center justify-center bg-[#e9eef4] px-1 text-center uppercase text-slate-600" style={{ gridColumn: "span 2" }}><span className="text-[9px] font-black tracking-wide">Games remaining</span><strong className="mt-1 text-3xl font-black leading-none text-slate-950">{Math.max(0, bowlGames.length - bowlGradedGames)}</strong></span>{bowlGames.map((game, index) => <span className={`border-b-2 border-[#1d1d1f] bg-[#e9eef4] px-2 py-2 text-center text-[10px] leading-4 text-slate-700 ${bowlGameBoundaryClass(index)}`} data-bowl-game-index={index} key={`${game.id}-${index}`}><b className="block text-xs uppercase tracking-wide text-slate-900">{bowlName(game)}</b><span className="mt-1 block break-words font-bold" title={bowlTeam(game, "favorite")?.full_name}>{bowlTeamLabel(bowlTeam(game, "favorite"))}</span><span className="block font-mono font-black text-[#007e72]">{game.line?.locked_spread ?? "—"}</span><span className="block break-words font-bold" title={bowlTeam(game, "underdog")?.full_name}>{bowlTeamLabel(bowlTeam(game, "underdog"))}</span></span>)}<span className="border-b-2 border-[#1d1d1f] bg-[#e9eef4] px-1 py-2 text-center text-[10px] font-black uppercase tracking-wide text-slate-600">TB</span></div>
                {bowlRows.map((row, rowIndex) => { const rowFill = rowIndex % 2 ? "bg-[#e9eef4]" : "bg-[#f5f0e6]"; return <div className={`grid border-b border-[#91afd0] text-center text-xs ${rowFill}`} style={{ gridTemplateColumns: `3rem 6rem repeat(${bowlGames.length}, minmax(8rem, 1fr)) minmax(4.5rem, .55fr)` }} key={row.playerId}><span className={`bowl-standings-sticky sticky left-0 z-30 px-1 py-2 text-center font-mono font-black tabular-nums ${rowFill}`}>{row.wins}</span><span className={`bowl-standings-sticky sticky left-[3rem] z-30 truncate px-1 py-2 text-left font-serif font-bold ${rowFill}`} title={row.playerName}><PlayerTrophyName name={row.playerName} showTrophy={row.trophies?.some((title) => title.includes("Bowl Pool Champion"))} titles={row.trophies} /></span>{bowlGames.map((game, index) => { const result = bowlCell(row.playerId, game.id); const locked = bowlStandings?.privatePickMarkers?.some((pick) => pick.playerId === row.playerId && pick.game_id === game.id); const display = result === "·" && locked ? "🔒" : result; return <span className={`px-1 py-2 font-black ${bowlCellClass(display)} ${bowlGameBoundaryClass(index)}`} key={`${row.playerId}-${game.id}-${index}`}>{display}</span>; })}<span className="px-1 py-2 font-mono text-xs font-black tabular-nums text-slate-700">{row.tiebreakerTotal ?? "—"}</span></div>; })}
              </div>
            </div>
          </> : null}
        </section> : null}
