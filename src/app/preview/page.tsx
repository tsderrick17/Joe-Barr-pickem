  ...
  if (scenario.emailImage === "slate") {
    return <div aria-label="Sample Slate image included in this email" className="mt-6 overflow-hidden border border-[#b7aea0] bg-[#fffdf8]" role="img">
      ...
      {games.slice(0, 4).map((game, index) => <div className={`grid grid-cols-[4.8rem_minmax(0,1fr)_3.4rem_minmax(0,1fr)] items-center gap-1 border-b border-[#d8cfbf] px-3 py-2 text-[11px] ${index % 2 === 0 ? "bg-[#f0e5cf]" : ""}`} key={game.id}>
        <span className="font-bold text-slate-500">{game.kickoff.replace(" ET", "")}</span><strong className="pr-1 text-right">{game.left}</strong><strong className="slate-preview-spread text-center font-mono">{game.line.replace(/^[A-Z]+\s+/, "")}</strong><strong className="pl-1 text-left">{game.right}</strong>
      </div>)}
