...
function Helmet({ team, faces, unavailable }: { team: Team; faces: "left" | "right"; unavailable?: boolean }) {
  ...
  return <span aria-hidden="true" className={`relative block h-16 w-20 shrink-0 ${flipped ? "-scale-x-100" : ""} ${unavailable ? "grayscale opacity-60" : ""}`}>
...
export default function SurvivorPage() {
  ...
  async function load() {
    try {
      ...
      else setMessage(error instanceof Error ? error.message : "The Survivor Wire could not be loaded.");
    }
  }
  ...
  async function save() {
    ...
    try {
      ...
      setMessage(result.message ?? "Your Survivor pick has been saved.");
      await load();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Your Survivor pick could not be saved."); }
  ...
  return <main className="min-h-screen bg-[#f5f0e6] px-4 py-5 text-[#171719] sm:px-5 sm:py-8 md:px-10"><div className="mx-auto max-w-3xl">
    <header className="border-b-2 border-[#1d1d1f] pb-5"><p className="text-xs font-bold tracking-[.2em] text-slate-600">STRAIGHT-UP SURVIVOR</p><h1 className="mt-1 font-serif text-4xl font-black">The Survivor Wire</h1><p className="mt-2 text-slate-700">{data.week.name} · Pick one outright winner. A team may only be used once.</p>{data.byeTeams.length ? <p className="mt-2 text-xs font-bold tracking-[.12em] text-slate-600">BYE THIS WEEK: {data.byeTeams.join(", ")}</p> : null}</header>
