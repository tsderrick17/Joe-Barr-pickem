...
export default function SurvivorPage() {
  ...
  return <main className="min-h-screen bg-[#f5f0e6] px-4 py-5 text-[#171719] sm:px-5 sm:py-8 md:px-10"><div className="mx-auto max-w-3xl">
    ...
    <section className={`mt-7 border-t-4 border-[#171719] ${eliminated || complete ? "grayscale" : ""}`}><div className="flex items-center justify-between border-b border-[#171719] py-3"><h2 className="font-serif text-2xl font-black">This week&apos;s matchups</h2><span className="text-[11px] font-bold tracking-[.16em] text-slate-600">{eliminated || complete ? "VIEW ONLY" : "SELECT ONE"}</span></div><div className="grid grid-cols-[1fr_2rem_1fr] border-b border-[#d1c9ba] text-center text-[10px] font-bold tracking-[.16em] text-slate-600"><span className="py-2">AWAY</span><span /><span className="py-2">HOME</span></div><div className="divide-y divide-[#c9c1b2]">
      {data.games.map((game) => { const started = new Date(game.kickoffAt) <= new Date(); const teams = [game.awayTeam, game.homeTeam]; return <article className="py-1" key={game.id}><div className="grid grid-cols-[minmax(0,1fr)_2rem_minmax(0,1fr)] items-center">{teams.map((team, index) => { const chosen = selected?.teamId === team.id; const official = data.entry.pick?.selected_team_id === team.id; const unavailable = isSurvivorTeamUnavailable({ teamId: team.id, usedTeamIds: data.usedTeamIds, savedPickTeamId: data.entry.pick?.selected_team_id ?? null, gameStarted: started, entryEliminated: eliminated || complete }); return <button aria-label={`Choose ${team.name}`} aria-pressed={chosen} className={`survivor-matchup-team grid min-h-20 items-center gap-2 px-1 py-2 disabled:cursor-not-allowed sm:px-3 ${index === 0 ? "grid-cols-[minmax(0,1fr)_auto]" : "col-start-3 grid-cols-[auto_minmax(0,1fr)]"}`} disabled={unavailable} key={team.id} onClick={() => setSelected(chosen ? null : { gameId: game.id, teamId: team.id })} title={unavailable ? `${team.name} is unavailable` : `Choose ${team.name}`} type="button">{index === 0 ? <><span className="text-right text-xs font-bold leading-4 text-zinc-800 sm:text-sm">{team.name}</span><SurvivorPokerChip abbreviation={team.abbreviation} official={official} selected={chosen} teamName={team.name} unavailable={unavailable} /></> : <><SurvivorPokerChip abbreviation={team.abbreviation} official={official} selected={chosen} teamName={team.name} unavailable={unavailable} /><span className="text-left text-xs font-bold leading-4 text-zinc-800 sm:text-sm">{team.name}</span></>}</button>; })}<span aria-hidden="true" className="col-start-2 row-start-1 text-center font-serif text-[10px] font-black italic tracking-[.08em] text-slate-400">VS</span></div></article>; })}
    </div></section>
