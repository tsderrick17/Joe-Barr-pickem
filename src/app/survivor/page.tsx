...
function Helmet({ team, faces, unavailable }: { team: Team; faces: "left" | "right"; unavailable?: boolean }) {
  ...
  return <span aria-hidden="true" className={`relative block h-14 w-20 shrink-0 ${flipped ? "-scale-x-100" : ""} ${unavailable ? "grayscale opacity-55" : ""}`}>
    <Image alt="" className="absolute inset-0 h-full w-full scale-[1.55] object-contain" height={56} src="/survivor-helmet-master.png" width={80} />
    <span className="absolute inset-[5%] scale-[1.55]" style={{ backgroundColor: helmetColors[team.abbreviation] ?? "#fff", clipPath: "polygon(18% 86%, 13% 76%, 10% 59%, 11% 40%, 18% 24%, 31% 13%, 47% 8%, 62% 11%, 74% 20%, 82% 32%, 84% 45%, 76% 47%, 69% 42%, 66% 59%, 59% 71%, 49% 80%, 34% 83%)", mixBlendMode: "color" }} />
