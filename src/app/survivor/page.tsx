...
function TeamMark({ team, unavailable }: { team: Team; unavailable?: boolean }) {
  return <span aria-hidden="true" className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-black/15 bg-white p-1.5 shadow-sm ${unavailable ? "grayscale opacity-55" : ""}`} style={{ boxShadow: `inset 0 3px 0 ${helmetColors[team.abbreviation] ?? "#171719"}` }}>
    <Image alt="" className="h-full w-full object-contain" height={32} src={`/team-logos/${team.abbreviation}.png`} width={32} />
  </span>;
}
