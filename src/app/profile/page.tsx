  ...
  return <main className="min-h-screen bg-[#f5f0e6] px-4 py-8 text-[#171719] sm:px-6 sm:py-12"><div className="mx-auto max-w-2xl">
    ...
    <form className="mt-8 border-y-2 border-[#1d1d1f] py-6" onSubmit={save}>
      ...
      {enabled ? <div className="mt-6 space-y-7">
        <section><h2 className="text-sm font-black tracking-[.14em] text-[#171719]">RECOMMENDED</h2><p className="mt-1 text-sm text-slate-600">The three essentials for following the pool without being chased down.</p><div className="mt-2 divide-y divide-zinc-200 border-y border-zinc-200"><Choice checked={preferences.weeklyRecap} note="A friendly weekly wrap-up with the final slate, standings, and Survivor result while it is active." onChange={(checked) => update({ weeklyRecap: checked })} title="Weekly recap" /><Choice checked={preferences.finalLines} note="Official lines at 8:30 AM on every gameday." onChange={(checked) => update({ finalLines: checked, ...(checked ? { sundayFinalLines: false } : {}) })} title="ALL final gameday lines" /><Choice checked={preferences.sundayFinalLines} disabled={preferences.finalLines} note={preferences.finalLines ? "Included in your ALL final gameday lines choice." : "A Sunday-only alternative at 8:30 AM."} onChange={(checked) => update({ sundayFinalLines: checked })} title="Sunday-only final lines" /><Choice checked={preferences.pickDue} note="A courteous reminder ONLY when you still have a selection to make." onChange={(checked) => update({ pickDue: checked })} title="Selections still to be made" /></div></section>
