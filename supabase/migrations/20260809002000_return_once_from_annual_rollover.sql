-- RETURN QUERY appends to a PL/pgSQL result set but does not exit the
-- function. A newly created season therefore returned both the created=true
-- row and the retry created=false row. Preserve the same safe rollover logic
-- while returning exactly one result to every caller.
...
begin
  ...
  if existing_id is null then
    ...
    join public.seasons as season on season.id = period.season_id
    where season.year = template_year;

    if template_count = 0 then
      raise exception 'Cannot create season % without scoring-period template from season %.',
        target_year, template_year;
    end if;

    insert into public.seasons(year, state)
    values (target_year, 'preseason')
...
revoke all on function public.ensure_annual_season_rollover(timestamptz)
  from public, anon, authenticated;
grant execute on function public.ensure_annual_season_rollover(timestamptz)
  to service_role;
