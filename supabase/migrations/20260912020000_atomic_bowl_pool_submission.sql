-- Save the entry and all unlocked picks as one database transaction.
...
declare target_entry_id uuid; first_kickoff timestamptz; selection record; is_commissioner boolean;
begin
  select coalesce(is_commissioner, false) into is_commissioner from players where id = target_player_id;
  ...
  if not target_opted_in then
    if first_kickoff is not null and evaluated_at >= first_kickoff and not is_commissioner then raise exception 'Bowl Pool opt-out closed at the first kickoff.'; end if;
  ...
  if first_kickoff is not null and evaluated_at >= first_kickoff and not is_commissioner and (target_entry_id is null or not exists (select 1 from bowl_pool_entries where id=target_entry_id and status in ('active','complete'))) then raise exception 'Bowl Pool entry closed at the first kickoff.'; end if;
