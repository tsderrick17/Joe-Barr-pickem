-- Run Bowl Pool schedule, line, score, and settlement work on its own cadence.
-- The NFL worker is intentionally faster now, so Bowl work must not depend on
-- its invocation minutes to retain the intended 15-minute cadence.
do $migration$
declare
...
begin
  if to_regprocedure('cron.unschedule(integer)') is not null then
    execute $sql$
      ...
      from cron.job
      where jobname = 'refresh-bowl-pool-every-fifteen-minutes'
    $sql$;
  ...
  if to_regprocedure('cron.schedule(text,text,text)') is not null then
    execute format('select cron.schedule(%L, %L, %L)', 'refresh-bowl-pool-every-fifteen-minutes', '*/15 * * * *', command_text);
  elsif to_regprocedure('cron.schedule(text,text)') is not null then
    execute format('select cron.schedule(%L, %L)', '*/15 * * * *', command_text);
  end if;
end
$migration$;
...
as $$
  with required_jobs(job_name, job_label, expected_schedule, expected_path) as (
    values
      ...
      ('refresh-bowl-pool-every-fifteen-minutes', 'Bowl Pool worker every fifteen minutes', '*/15 * * * *', '/api/cron/sync-bowl-scores'),
