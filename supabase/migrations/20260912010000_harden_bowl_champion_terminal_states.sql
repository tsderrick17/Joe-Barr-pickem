-- Treat cancelled and no-contest Bowl games as terminal so a disruption
-- cannot prevent the season from completing or leave the champion unsealed.
...
begin
  ...
  with totals as (
    ...
    where entry.season_id = target_season_id and entry.status in ('active', 'complete') and entry.championship_total_guess is not null
    ...
    select * from ranked where wins = top_wins and difference = (select min(difference) from ranked where wins = top_wins)
  )
