create or replace function public.prevent_completed_scoring_period_changes()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists prevent_completed_scoring_period_changes
  on public.scoring_periods;

create trigger prevent_completed_scoring_period_changes
before update on public.scoring_periods
for each row
execute function public.prevent_completed_scoring_period_changes();
...
drop trigger if exists prevent_completed_game_changes
  on public.games;

create trigger prevent_completed_game_changes
before insert or update or delete on public.games
for each row
execute function public.prevent_completed_game_changes();
...
drop trigger if exists prevent_completed_pick_changes
  on public.picks;

create trigger prevent_completed_pick_changes
before insert or update or delete on public.picks
for each row
execute function public.prevent_completed_pick_changes();
...
begin
  ...
  if exists (
    select 1
    from public.games
    join public.scoring_periods
      on scoring_periods.id = games.scoring_period_id
    where games.id = target_game_id
      and scoring_periods.status = 'complete'
  ) and (tg_op in ('INSERT', 'DELETE') or new is distinct from old) then
...
drop trigger if exists prevent_completed_line_changes
  on public.game_lines;

create trigger prevent_completed_line_changes
before insert or update or delete on public.game_lines
for each row
execute function public.prevent_completed_line_changes();
